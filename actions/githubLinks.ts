...
export async function putGithubStats(username: string, user_id: string,qr:string) {
...
//       .select("*");
    ...
    const errorObj = {
      success: false,
      errorMessage: "Error in fetching Github data",
    };
    return errorObj;
  }
}
...
export async function getGithubStats(user_id: string) {
  const { data, error } = await supabase
    .from("Leetcode")
    .select("*")
    .eq("user_id", user_id)
